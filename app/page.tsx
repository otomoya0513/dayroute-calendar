...
export default function Home() {
  ...
  return <main className="app-shell">
    <header className="topbar">
      ...
      <div className="top-actions"><button id="saved-places-button" className="place-button" onClick={() => setShowPlaces(true)}>☆ 登録地点</button><button id="add-event-button" className="add-button" onClick={() => setShowForm(true)}>＋ 予定を追加</button></div>
    </header>
    ...
    <section className="summary-bar">
      ...
      <div className="summary-side"><div className="view-switch" aria-label="表示単位">{viewModes.map(mode => <button id={`view-${mode.id}`} key={mode.id} className={viewMode === mode.id ? "active" : ""} onClick={() => setViewMode(mode.id)}>{mode.label}</button>)}</div>
    ...
    {viewMode === "day" ? <section className="workspace">
      <div className="schedule-panel"><div className="panel-heading"><div><p className="eyebrow">SCHEDULE</p><h2>時間順の予定</h2></div><button className="icon-button" onClick={() => setShowForm(true)} aria-label="予定を追加">＋</button></div>
        <div className="timeline">
          {startLocation && <><div id="start-point-card" className="start-point-card"><span>S</span><div><small>START POINT</small><strong>{startLocation.name}</strong><p>{startLocation.address}</p></div><button onClick={() => setShowPlaces(true)}>変更</button></div>{startRoute && <TravelRow route={startRoute} />}</>}
    ...
    {notice && <div id="dayroute-toast" className="toast" role="status">✓ {notice}</div>}
  </main>;
}
...
function EventForm({ selectedDate, favorites, onClose, onSubmit, onSaveFavorite }: { selectedDate: string; favorites: FavoritePlace[]; onClose: () => void; onSubmit: (event: ScheduleEvent) => void; onSaveFavorite: (place: PlaceSuggestion, label: string) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><form id="event-form" className="event-form" onSubmit={submit} onMouseDown={event => event.stopPropagation()}><div className="form-heading"><div><p className="eyebrow">NEW SCHEDULE</p><h2>予定を追加</h2></div><button type="button" onClick={onClose} aria-label="閉じる">×</button></div>
    <label>予定名<input id="event-title" autoFocus value={title} onChange={event => setTitle(event.target.value)} placeholder="例：展示会を見に行く" required /></label><div className="form-grid"><label>日付<input id="event-date" type="date" value={date} onChange={event => setDate(event.target.value)} required /></label><div className="place-field-wrap"><PlaceSearch testId="event-place-search" label="場所" value={place} onSelect={(selected) => { setPlace(selected); setError(""); }} />{favorites.length > 0 && <div className="favorite-chips">{favorites.map(item => <button type="button" key={item.id} onClick={() => setPlace(item)}>☆ {item.label}</button>)}</div>}{place && !favorites.some(item => item.id === place.id) && <button type="button" className="quick-favorite" onClick={() => onSaveFavorite(place, place.name)}>☆ この場所をお気に入りに追加</button>}{error && <span className="field-error">{error}</span>}</div><label>開始<input id="event-start" type="time" value={start} onChange={event => setStart(event.target.value)} required /></label><label>終了<input id="event-end" type="time" value={end} onChange={event => setEnd(event.target.value)} min={start} required /></label><label>カテゴリ<select id="event-category" value={category} onChange={event => setCategory(event.target.value as Category)}>{categories.map(item => <option key={item}>{item}</option>)}</select></label><label className="check-label"><input type="checkbox" checked={flexible} onChange={event => setFlexible(event.target.checked)} /><span>時間・場所を調整できる予定</span></label></div>
    <div className="form-actions"><button type="button" className="secondary-button" onClick={onClose}>キャンセル</button><button id="submit-event-button" className="submit-button">予定を追加</button></div></form></div>;
}
...
function PlaceManager({ startLocation, favorites, onClose, onSetStart, onSaveFavorite, onRemoveFavorite }: { startLocation: PlaceSuggestion | null; favorites: FavoritePlace[]; onClose: () => void; onSetStart: (place: PlaceSuggestion | null) => void; onSaveFavorite: (place: PlaceSuggestion, label: string) => void; onRemoveFavorite: (id: string) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><div id="place-manager" className="place-manager" onMouseDown={event => event.stopPropagation()}><div className="form-heading"><div><p className="eyebrow">SAVED PLACES</p><h2>開始地点・お気に入り</h2></div><button onClick={onClose} aria-label="閉じる">×</button></div>
    ...
    <section className="add-place-section"><h3>地点を検索</h3><PlaceSearch testId="manager-place-search" label="" value={place} onSelect={selected => { setPlace(selected); setLabel(selected?.name ?? ""); }} />{place && <div className="place-actions"><label>お気に入りの表示名<input id="favorite-label" value={label} onChange={event => setLabel(event.target.value)} placeholder="例：自宅" /></label><div><button id="set-start-location-button" onClick={() => onSetStart(place)} className="secondary-button">開始地点に設定</button><button id="save-favorite-button" onClick={() => onSaveFavorite(place, label)} className="submit-button">お気に入りに保存</button></div></div>}</section>
  </div></div>;
}

function PlaceSearch({ testId, label, value, onSelect }: { testId: string; label: string; value: PlaceSuggestion | null; onSelect: (place: PlaceSuggestion | null) => void }) {
  ...
  return <label className="place-field">{label}<div className={`place-search ${value ? "selected" : ""}`}><span className="search-mark">⌕</span><input id={`${testId}-input`} value={query} onChange={event => { setQuery(event.target.value); onSelect(null); setSuggestions([]); setSearching(event.target.value.trim().length >= 2); setError(""); }} placeholder="駅名・施設名・住所を入力" autoComplete="off" role="combobox" aria-controls={`${testId}-results`} aria-expanded={suggestions.length > 0} aria-autocomplete="list" />{searching && <span className="search-spinner" />}{value && <span className="selected-check">✓</span>}</div>{suggestions.length > 0 && <div className="suggestion-list" id={`${testId}-results`} role="listbox">{suggestions.map(place => <button type="button" key={place.id} role="option" aria-selected="false" onClick={() => choose(place)}><span className="suggestion-icon">{place.type === "駅" ? "🚉" : "⌖"}</span><span className="suggestion-copy"><strong>{place.name}</strong><small>{place.type}・{place.address}</small></span></button>)}</div>}{!searching && query.trim().length >= 2 && !value && suggestions.length === 0 && !error && <span className="field-hint">該当する候補がありません。別の名称や住所をお試しください。</span>}{error && <span className="field-error">{error}</span>}<span className="place-credit">検索データ © OpenStreetMap contributors</span></label>;
}
